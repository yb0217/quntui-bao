...
class QuntuiBot:
    ...
    async def handle_new_member(self, event):
        ...
        try:
            ...
            if welcome_msg:
                ...
                # 发送欢迎消息（使用 HTML 解析模式）
                sent_msg = await self.client.send_message(chat_id, final_welcome_msg, parse_mode='html')
                print(f"👋 欢迎消息已发送到群 {chat_id}, msg_id={sent_msg.id}")
                
                # 1分钟后自动删除欢迎消息
                asyncio.create_task(self.delete_message_after(chat_id, sent_msg.id, 60, 'welcome'))
    ...
    async def delete_message_after(self, chat_id, message_id, delay_seconds, msg_type='message'):
        """延迟删除消息"""
        await asyncio.sleep(delay_seconds)
        try:
            await self.client.delete_messages(chat_id, [message_id])
            print(f"🗑️ {msg_type}消息已删除: chat={chat_id}, msg_id={message_id}")
        except Exception as e:
            # 消息可能已被删除或不存在，忽略错误
            print(f"⚠️ 删除{msg_type}消息失败(可能已被删): chat={chat_id}, msg_id={message_id}, error={e}")
            ...
            try:
                ...
                while send_index < groups_count:
                    ...
                    try:
                        ...
                        print(f"💾 已保存广告消息ID: group={group_id}, msg_id={sent_msg.id}")
